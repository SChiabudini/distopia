...
if ( ! class_exists( 'PSCW_PRODUCT_SIZE_CHART_F_WOO_Design' ) ) {
	class PSCW_PRODUCT_SIZE_CHART_F_WOO_Design {
		...
		public function register_scripts() {
			...
			if ( ( $current_screen == 'pscw-size-chart' ) || ( $current_screen == 'edit-pscw-size-chart' ) ) {
			...
			if ( $current_screen == 'pscw-size-chart' ) {
				...
				wp_enqueue_script( 'size_chart_perfect_scrollbar_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/perfect-scrollbar.min.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION );
				...
				wp_enqueue_style( 'size_chart_perfect_scrollbar_css', PSCW_SIZE_CHART_PLUGIN_URL . 'css/perfect-scrollbar.css', '', PSCW_SIZE_CHART_VERSION );
				if ( WP_DEBUG ) {
					...
					wp_enqueue_script( 'woo_size_chart_design_js', PSCW_SIZE_CHART_PLUGIN_URL . 'js/woo_size_chart_design.js', array( 'jquery' ), PSCW_SIZE_CHART_VERSION );
				} else {
		...
		public function woo_sc_size_chart_design( $post_id ) {
            ...
            <input type="hidden" name="prevent_delete_meta_movetotrash" id="prevent_delete_meta_movetotrash" value="<?php echo wp_create_nonce( plugin_basename( __FILE__ ) . $post_id->ID ); ?>">
                ...
                <tr>
                    ...
                    <td>
                        <select name="woo_sc_hide" id="woo_sc_hide" class="vi-ui dropdown woo_sc_table_tools">
                            <option value="none" <?php selected( $meta_box_data['hide'], 'none' ); ?>><?php esc_html_e( 'None', 'product-size-chart-for-woo' ) ?></option>
                            <option value="hide_table" <?php selected( $meta_box_data['hide'], 'hide_table' ); ?>><?php esc_html_e( 'Table', 'product-size-chart-for-woo' ) ?></option>
                            <option value="hide_image" <?php selected( $meta_box_data['hide'], 'hide_image' ); ?>><?php esc_html_e( 'Image', 'product-size-chart-for-woo' ) ?></option>
                            <option value="hide_all" <?php selected( $meta_box_data['hide'], 'hide_all' ); ?>><?php esc_html_e( 'Hide all', 'product-size-chart-for-woo' ) ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
		...
		function save_data_meta_box( $post_id ) {
			...
			if (
				...
				( isset( $_POST['prevent_delete_meta_movetotrash'] ) && ( ! wp_verify_nonce( $_POST['prevent_delete_meta_movetotrash'], plugin_basename( __FILE__ ) . $post_id ) ) )
			) {
				return $post_id;
			}
			...
			if ( isset( $_POST['woo_sc_table_template'] ) ) {
				$meta_box_table_template_value = sanitize_text_field( $_POST['woo_sc_table_template'] );
			}
			...
			if ( isset( $_POST["woo_sc_even_columns_color"] ) ) {
				$meta_box_even_columns_color_value = sanitize_text_field( $_POST["woo_sc_even_columns_color"] );
			}
			if ( isset( $_POST["woo_sc_cell_style"] ) ) {
				$woo_sc_cell_style = sanitize_text_field( $_POST["woo_sc_cell_style"] );
			}
			...
			if ( isset( $_POST["woo_sc_odd_columns_color"] ) ) {
				$meta_box_odd_columns_color_value = sanitize_text_field( $_POST["woo_sc_odd_columns_color"] );
			}
			...
			if ( isset( $_POST["woo_sc_odd_columns_text_color"] ) ) {
				$meta_box_odd_columns_text_color_value = sanitize_text_field( $_POST["woo_sc_odd_columns_text_color"] );
			}
			...
			if ( isset( $_POST["woo_sc_even_columns_text_color"] ) ) {
				$meta_box_even_columns_text_color_value = sanitize_text_field( $_POST["woo_sc_even_columns_text_color"] );
			}
			//border style
